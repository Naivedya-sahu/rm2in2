...
// Wacom digitizer ranges
#define WACOM_MAX_X 20967
...
static inline int display_to_wacom_x(int display_x) {
    int flippedx = RM2_WIDTH - display_x;  // invert horizontal axis
    return (int)((long)flippedx * WACOM_MAX_X / RM2_WIDTH);
}

static inline int display_to_wacom_y(int display_y) {
    int flippedy = RM2_HEIGHT - display_y;  // invert vertical axis
    return (int)((long)flippedy * WACOM_MAX_Y / RM2_HEIGHT);
}

// // Convert display coords to Wacom coords
// // NOTE: X and Y are SWAPPED
// static inline int display_to_wacom_x(int display_x) {
//     return (int)((long)display_x * WACOM_MAX_X / RM2_WIDTH);
// }

// static inline int display_to_wacom_y(int display_y) {
//     return (int)((long)display_y * WACOM_MAX_Y / RM2_HEIGHT);
// }
...
static void queue_event(struct input_event *ev) {
    ...
    if ((queue_tail + 1) % MAX_QUEUE_SIZE == queue_head) {
        fprintf(stderr, "[INJECT] Queue full\n");
...
static void* fifo_reader_thread(void* arg) {
    fprintf(stderr, "[INJECT] Server ready\n");
        ...
        while ((n = read(fd, buf, sizeof(buf) - 1)) > 0) {
            ...
            if (sscanf(buf, "%s %d %d", cmd, &x, &y) >= 1) {
                ...
                if (strcmp(cmd, "PEN_DOWN") == 0) {
                    ...
                    fprintf(stderr, "[INJECT] Cursor: %d %d\n", last_pen_x, last_pen_y);
                }
            }
        }
...
ssize_t read(int fd, void *buf, size_t count) {
    ...
    if (wacom_fd == -1 && is_wacom_device(fd)) {
        wacom_fd = fd;
        fprintf(stderr, "[INJECT] Wacom detected on fd %d\n", fd);
...
static void init(void) {
    fprintf(stderr, "[INJECT] Hook loaded\n");
}
